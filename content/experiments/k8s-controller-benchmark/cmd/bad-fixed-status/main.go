...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		Scheme: scheme,
		Cache:  cache.Options{DefaultNamespaces: map[string]cache.Config{"bad-fixed-status": {}}},
	...
	if err = (&controller.BadFixedStatusReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to set up bad-fixed-status controller")
		os.Exit(1)
	}
	...
	setupLog.Info("starting bad-fixed-status controller (5 workers, predicate OFF)")
