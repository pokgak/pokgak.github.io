...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		Scheme: scheme,
		Cache:  cache.Options{DefaultNamespaces: map[string]cache.Config{"bad-fixed-single": {}}},
	...
	if err = (&controller.BadFixedSingleReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to set up bad-fixed-single controller")
		os.Exit(1)
	}
	...
	setupLog.Info("starting bad-fixed-single controller (1 worker, predicate OFF)")
