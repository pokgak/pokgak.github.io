...
**[autoresearch](https://github.com/karpathy/autoresearch)** by Karpathy is the most direct parallel to the overnight experiment runner use case. The setup: give an agent a single training file (`train.py`) and a fixed 5-minute budget per run, let it modify the code, train, measure val loss, keep or discard, and repeat. ~100 experiments while you sleep. Like `/loop`, it runs within a single session — context accumulates across experiments, so the agent remembers what it tried and why it kept or discarded each run. The `program.md` spec is instructions for how to run one experiment, not a mechanism to reconstruct state; the agent doesn't need it to remember what happened because it was there. The key design difference from my approach is the fixed time budget — it makes every experiment directly comparable regardless of what the agent changes (architecture, optimizer, batch size). My `/loop` approach is less structured: Claude decides what to test next based on Darwinian selection rather than modifying a single canonical file. autoresearch is tighter by design; `/loop` trades that tightness for flexibility across different experiment types.
