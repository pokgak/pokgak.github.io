...
- **Server-side apply is identity-keyed** → adoption of existing resources is a no-op when the manifest matches live; a `pull_request_target` dry-run diff is a safe, accurate pre-merge check *if* it only ever runs `diff`.
